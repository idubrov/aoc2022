...
fn bfs(winds: &Winds, initial: PathState, end: Pos2) -> usize {
...
fn solve(path: &str) -> (usize, usize) {
  ...
  for y in 0..h {
    winds.lefts.push(scan(&map, Pos2::new(1, 1 + y), Dir2::new(1, 0), b'<', w));
    winds.rights.push(scan(&map, Pos2::new(1, 1 + y), Dir2::new(1, 0), b'>', w));
  }
  for x in 0..w {
    winds.ups.push(scan(&map, Pos2::new(1 + x, 1), Dir2::new(0, 1), b'^', h));
    winds.downs.push(scan(&map, Pos2::new(1 + x, 1), Dir2::new(0, 1), b'v', h));
  }
  let start = Pos2::new(0, -1);
  let end = Pos2::new(w - 1, h);
  let start_target = Pos2::new(0, 0);
  let end_target = Pos2::new(w - 1, h - 1);
  let first = bfs(&winds, PathState { pos: start, step: 0 }, end_target) + 1;
  let back = bfs(&winds, PathState { pos: end, step: first }, start_target) + 1;
  let second = bfs(&winds, PathState { pos: start, step: back }, end_target) + 1;
  (first, second)
}
